...
void SiarController::loop() {
  ...
  if (operation_mode > 0) {
    if (operation_mode == 100) {
      ...
      cmd_vel_msg.angular.z *= mult * (1 + _conf.v_mult * mult); // mult belongs to [0,1] if mult near 0 --> not apply _conf.v_mult. If near 1 --> apply it 
...
void SiarController::statusCallback(const siar_driver::SiarStatus& msg)
{
  ...
  if (fabs(new_width - _conf.robot_width) > width_thres && cmd_eval != NULL ) {
    cmd_eval->setWidthElec(new_width, msg.electronics_x);
